...
for i=1:times
    ...
    [PositionChart,SettleAccount]=KnockOutCallHedging(OpenList,CloseList,SettleList,K,H,E,sigma,rf,rp,netprincipal,period,interval,positionrate,depositrate,tradefee,tradedays);
    managementfee=(managerate/tradedays)*sum(SettleAccount(1:period,10),1);   %基金管理费按每日净值成比例计提
    trustfee=(trusteerate/tradedays)*sum(SettleAccount(1:period,10),1);   %基金托管费按每日净值成比例计提
    endvalue=SettleAccount(period,10)-managementfee-trustfee;
