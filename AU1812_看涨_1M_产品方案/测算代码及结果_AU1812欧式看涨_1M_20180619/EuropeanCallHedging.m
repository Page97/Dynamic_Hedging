...
function [PositionChart,SettleAccount]=EuropeanCallHedging(OpenList,CloseList,SettleList,K,sigma,rf,rp,netprincipal,feerate,period,interval,positionrate,depositrate,tradefee,tradedays)
...
delta0=EuropeanCallDelta(S0,K,sigma,rf,period/tradedays);   %期初的delta
...
for i=2:period-1
    ...
    if mod(i-1,interval)==0    %判断是否为调仓日
        delta=EuropeanCallDelta(CloseList(i-1,2),K,sigma,rf,(period-i+1)/tradedays); %以调仓日前日收盘价作为对冲价格并计算Delta
